...
update(){
	...
	echo ""
	url="https://raw.githubusercontent.com/Audionut/FreshJR_QOS/master/FreshJR_QOS.sh"
	...
	echo ""
	echo "Curl Output:"
	curl "https://raw.githubusercontent.com/Audionut/FreshJR_QOS/master/FreshJR_QOS.sh" -o /jffs/scripts/FreshJR_QOS --create-dirs && curl "https://raw.githubusercontent.com/Audionut/FreshJR_QOS/master/FreshJR_QoS_Stats.asp" -o "${webpath}" && sh /jffs/scripts/FreshJR_QOS -install
	exit
}
...
case "$arg1" in
 ...
 'install'|'enable')															## INSTALLS AND TURNS ON SCRIPT
	...
	if [ "$(uname -o)" == "ASUSWRT-Merlin" ] ; then				  #Mounts webpage on RMerlin v382+
		...
		if [ "${CV}" -ge "382" ] ; then
			if ! [ "${webpath}" -ef "/www/QoS_Stats.asp" ] ; then
				mount -o bind "/jffs/scripts/FreshJR_QOS/FreshJR_QoS_Stats.asp" "/www/QoS_Stats.asp"
			fi
		#elif [ "${CV}" = "384" ] && [ ${MV} -ge "9" ] ; then
		fi
	fi
